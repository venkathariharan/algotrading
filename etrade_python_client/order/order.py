...
class Order:
    ...
    def preview_order(self):
        ...
        :return: preview_id if successful, None otherwise
        """
        ...
        # Display order summary before preview
        print("\n" + "-"*80)
        print("ORDER SUMMARY:")
        print("-"*80)
        print("Symbol: " + order["symbol"])
        print("Action: " + order["order_action"])
        print("Quantity: " + str(order["quantity"]))
        print("Price Type: " + order["price_type"])
        if order["price_type"] == "LIMIT":
            print("Limit Price: $" + str(order["limit_price"]))
        print("Order Term: " + order["order_term"])
        print("-"*80)
        ...
        # Handle limit_price - use empty string for MARKET orders
        limit_price_value = order["limit_price"] if order["price_type"] == "LIMIT" else ""
        
        # Ensure symbol is clean and uppercase
        symbol_to_send = str(order["symbol"]).strip().upper()
        
        # Debug: Print what we're about to send
        print("\nDEBUG - Order details being sent:")
        print("  Symbol: " + symbol_to_send)
        print("  Action: " + str(order["order_action"]))
        print("  Quantity: " + str(order["quantity"]))
        print("  Price Type: " + str(order["price_type"]))
        print("  Limit Price: " + str(limit_price_value))
        
        # Store original symbol for comparison (use symbol_to_send as the reference)
        original_symbol = symbol_to_send
        
        payload = payload.format(order["client_order_id"], order["price_type"], order["order_term"],
                                 limit_price_value, symbol_to_send, order["order_action"], order["quantity"])
        
        # Debug: Log the order details and show the XML payload
        logger.debug("Order details - Symbol: %s, Action: %s, Quantity: %s, Price Type: %s, Limit Price: %s", 
                     order["symbol"], order["order_action"], order["quantity"], order["price_type"], limit_price_value)
        print("\nDEBUG - XML Payload being sent:")
        print(payload)
                    ...
                    if orders is not None and "Instrument" in orders:
                        for instrument in orders["Instrument"]:
                            ...
                            if instrument is not None and "Product" in instrument \
                                ...
                                symbol_from_api = instrument["Product"]["symbol"]
                                print("Symbol: " + symbol_from_api)
                                # Warn if symbol doesn't match what we sent
                                if symbol_from_api != original_symbol:
                                    print("\n*** WARNING: Symbol mismatch! ***")
                                    print("  Sent: " + original_symbol)
                                    print("  Received: " + symbol_from_api)
                                    print("  This may indicate an API issue or cached response.")
    ...
    def place_order(self, preview_id):
        """
        Places an order using the preview ID from a preview order
        
        :param self: Pass in authenticated session and information on selected account
        :param preview_id: Preview ID from preview order response
        :return: order_id if successful, None otherwise
        """
        if preview_id is None:
            print("Error: No preview ID provided. Please preview the order first.")
            return None
        ...
        payload = payload.format(preview_id)
        ...
        if response is not None and response.status_code == 200:
            ...
            if data is not None and "PlaceOrderResponse" in data and "OrderIds" in data["PlaceOrderResponse"]:
                order_ids = []
                for order_id_obj in data["PlaceOrderResponse"]["OrderIds"]:
                    order_id = order_id_obj.get("orderId")
                    order_ids.append(order_id)
                    print("\nOrder successfully placed!")
                    print("Order ID: " + str(order_id))
                
                if data["PlaceOrderResponse"].get("Order") is not None:
                    order_details = data["PlaceOrderResponse"]["Order"]
                    if isinstance(order_details, list):
                        order_details = order_details[0]
                    
                    if "Instrument" in order_details:
                        for instrument in order_details["Instrument"]:
    ...
    def place_order_flow(self):
        """
        Complete flow to preview and place an order
        ...
        preview_id = self.preview_order()
        
        if preview_id is None:
            print("\nOrder preview failed. Cannot place order.")
            return
        ...
        if confirm in ['yes', 'y']:
            order_id = self.place_order(preview_id)
            if order_id:
                print("\nOrder placed successfully with Order ID: " + str(order_id))
            else:
                print("\nFailed to place order.")
        else:
            print("\nOrder cancelled. Order was not placed.")
    ...
    def user_select_order(self):
        ...
        order["limit_price"] = ""
        if order["price_type"] == "LIMIT":
            while order["limit_price"] == "" or (not order["limit_price"].replace('.', '').isdigit() \
        ...
        while order["symbol"] == "" or order["symbol"].strip() == "":
            order["symbol"] = input("\nPlease enter a stock symbol: ").strip().upper()
    ...
    def view_orders_summary(self):
        """
        Calls orders API to display a summary of orders without interactive menu
        ...
        if response_executed.status_code == 204:
            ...
            orders = self.print_orders(data, "executed")
            if not orders:
                print("    None")
        ...
        if response_cancelled.status_code == 204:
            ...
            orders = self.print_orders(data, "cancelled")
            if not orders:
                print("    None")
        ...
        if response_rejected.status_code == 204:
            ...
            orders = self.print_orders(data, "rejected")
            if not orders:
                print("    None")
        ...
        if response_expired.status_code == 204:
            ...
            orders = self.print_orders(data, "expired")
            if not orders:
                print("    None")
